...
class CryptoSendService
{
    ...
    public function userAddresses($network)
    {
        try {
            ...
            return [
                'senderAddresses' => $this->tatumIo->getUserAddresses()
            ];
        } catch (Exception $e) {
           throw new CryptoSendException($e->getMessage());
        }
    ...
    public function sendCryptoFinal($network, $receiverAddress, $amount, $priority, $senderAddress)
    {
        try {
            ...
            $sendResponse = $this->tatumIo->sendCryptoToAddress($receiverAddress, $amount, $priority, $senderAddress);
    ...
    {
        $addressesArray = $this->tatumIo->getUserAddresses();

        $check = [];

        foreach ($addressesArray as $ad) {
            $check[$ad['address']] = $ad['address'];
        }

        if (!isset($check[$address])) {
    ...
    public function updateBalance($addresses)
    {
        $tatumWalletApiLog = [];

        $firstAddress = $addresses['senderAddresses'][0]['address'] ?? ''; 
   
        if (!empty($firstAddress)) {
            $tatumWalletApiLog = tatumGetWalletApiLog($firstAddress);
        }
        ...
        if (!empty($tatumWalletApiLog)) {
            ...
            $oldPayload = $tatumWalletApiLog->payload;

            $payloadArr = json_decode($oldPayload, true);

            foreach ($addresses['senderAddresses'] as $walletAddress) {
                $address = $walletAddress['address'];
                $balance = $this->tatumIo->getAddressBalance($address);

                foreach ($payloadArr as $key => $cryptoWallet) {
                    if ($cryptoWallet['address'] == $address) {
                        $payloadArr[$key]['balance'] = $balance;
                    }
                }
            }
            ...
            foreach ($payloadArr as $key => $cryptoWallet) {
                $balanceSum = bcadd($balanceSum, $cryptoWallet['balance'], 10);
            }

            $newPayload = json_encode($payloadArr);

            $tatumWalletApiLog->payload = $newPayload;
            $tatumWalletApiLog->save();
