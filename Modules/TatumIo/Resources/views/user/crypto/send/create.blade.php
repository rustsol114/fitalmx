...
<div class="bg-white pxy-62 shadow" id="crypto-send-create">
    ...
    <form method="post" action="{{ route('tatumio.user.crypto_send.confirm') }}" id="crypto-send-form">
        ...
        <div class="mt-28 param-ref">
            <label class="gilroy-medium text-gray-100 mb-2 f-15" for="priority">{{ __('Sender Address') }}</label>
            <div class="avoid-blink">
                <select class="select2 priority" data-minimum-results-for-search="Infinity" id="senderAddress" name="senderAddress">
                    @foreach ($senderAddresses as $key => $d)
                    <option value="{{ encrypt($d['address']) }}">{{ $d['address'] }} - {{ __('Balance') }}: {{ $d['balance'] }}</option>
                    @endforeach
                </select>
                <input type="hidden" id="unencryptedSenderAddress" value="{{ $senderAddresses[0]['address'] }}" />
            </div>
        </div>
...
<script type="text/javascript">
     $(document).ready(function() {
        ...
        var userId = '{{ auth()->id() }}';
        var senderAddress = document.getElementById('unencryptedSenderAddress').value;

        // Add event listener to update senderAddress on dropdown change
        document.getElementById('senderAddressSelect').addEventListener('change', function() {
            senderAddressInput.value = this.value; // Update the hidden input value with the selected option value
        });
