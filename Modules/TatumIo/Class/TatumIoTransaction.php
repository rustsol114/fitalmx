...
class TatumIoTransaction
{
    ...
    public function getUserPrivateKey($address = '')
    {
        $privateKey = '';

        if (!empty($address)) {
            foreach ($this->userCryptoWallet as $kkk) {
                if (isset($kkk->key) && $kkk->address == $address) {
                    $privateKey = $kkk->key;
                }
            }
        } else {
            if (is_array($this->userCryptoWallet) && isset($this->userCryptoWallet[0]->key)) {
                $privateKey = $this->userCryptoWallet[0]->key;
            } else {
                $privateKey = $this->userCryptoWallet->key;
            }
        }
    ...
    public function getUserAddress()
    {
        ...
        if (is_array($this->userCryptoWallet) && isset($this->userCryptoWallet[0]->address)) {
            $adress = $this->userCryptoWallet[0]->address;
        } else {
            $adress = isset($this->userCryptoWallet->address) ? $this->userCryptoWallet->address : '';
        }
    ...
    public function getUserAddresses()
    {
        ...
        if (is_array($this->userCryptoWallet)) {
            foreach ($this->userCryptoWallet as $data) {
                $adresses[] = [
    ...
    public function sendCryptoToAddress($receiver, $amount, $priority, $senderAddress)
    {
        return $this->cryptoSendProcess($senderAddress, $this->getUserPrivateKey($senderAddress), $receiver, $amount, $priority);
    }
    ...
    public function getReceiverAddressWalletUserId($receiverAddress)
    {
        return (new CryptoAssetApiLog())
            ...
            ->where('payload', 'like', "%$receiverAddress%")
            ->first('object_id');
    }

    public function createCryptoWalletLog($walletId, $userId, $network)
    {
        Queue::push(new CreateWalletsJob($walletId, $userId, $network));
    }
