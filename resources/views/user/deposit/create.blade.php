...
<div class="bg-white pxy-62 shadow" id="depositCreate">
    ...
    <form method="post" action="{{ route('user.deposit.confirm') }}" id="depositCreateForm">
        ...
        <div class="mt-28 param-ref">
            ...
            <div class="avoid-blink">
                <select class="select2" data-minimum-results-for-search="Infinity" name="currency_id" id="currency_id">
                    ...
                    <option data-type="{{ $currency['type'] }}" value="{{ $currency['id'] }}" @if (old('currency_id') && old('currency_id')==$currency['id']) {{ 'selected="selected"' }} @elseif (!empty(session('paymentData')) && session('paymentData')['currencyId']==$currency['id']) {{ 'selected="selected"' }} @elseif (empty(old('currency_id')) && empty(session('paymentData')) && $defaultWallet==$currency['id']) {{ 'selected="selected"' }} @endif>{{ $currency['code'] }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="mt-20 param-ref">
                    <label class="gilroy-medium text-gray-100 mb-2 f-15" for="bank_name">{{ __('Banks') }}</label>
                    <select class="select2" name="mx_bank_name">
                        @php
                        $selectedBank = isset(session('paymentData')['mx_bank_name']) ? old('mx_bank_name') : $defaultBankName;
                        @endphp
        ...
        <div class="mt-20 label-top">
            <label class="gilroy-medium text-gray-100 mb-2 f-15" for="stp_email">{{ __('Payment Description') }}</label>
            <input value="{{ session('paymentData')['payment_description'] ?? old('payment_description') }}" id="payment_description" class="form-control input-form-control apply-bg l-s2" type="text" class="form-control" name="payment_description" required>
            <span class=" custom-error"></span>
        </div>

        <div class="mt-20 label-top">
            <label class="gilroy-medium text-gray-100 mb-2 f-15" for="stp_email"> STP {{ __('Email') }}</label>
            <input value="{{ session('paymentData')['stp_email'] ?? old('stp_email') }}" id="stp_email" class="form-control input-form-control apply-bg l-s2" type="email" class="form-control" name="stp_email" required>
            <span class=" custom-error"></span>
        </div>

        <div class="mt-20 label-top">
            <label class="gilroy-medium text-gray-100 mb-2 f-15" for="stp_pass"> STP {{ __('Password') }}</label>
            <input value="{{ session('paymentData')['stp_pass'] ?? old('stp_pass') }}" id="stp_pass" class="form-control input-form-control apply-bg l-s2" type="password" class="form-control" name="stp_pass" required>
            <span class=" custom-error"></span>
        </div>
    ...
    <div id="balanceCheckMessage" class="alert alert-danger mb-2 mt-4" style="display: none;">
        <p id="serviceMessage" style="text-align: center;"></p>
    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        var buttonDefaultText = document.getElementById('depositCreateSubmitBtnText').innerText;

        $('#depositCreateForm').submit(function(event) {

            event.preventDefault();

            var stp_email = $('#stp_email').val();
            var stp_pass = $('#stp_pass').val();
            var amount = $('#amount').val();
            var csrfToken = $('meta[name="csrf-token"]').attr('content');

            document.getElementById('balanceCheckMessage').style.display = 'none';
            ...
            var checkInfoLink = "{{ route('user.deposit.info-from-bank') }}";
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success === false) {
                        document.getElementById('serviceMessage').innerText = response.message;
                        document.getElementById('balanceCheckMessage').style.display = 'block';

                        document.getElementById('depositCreateSubmitBtnText').innerText = buttonDefaultText;

                        $('#depositCreateSubmitBtn .spinner').addClass('d-none');
                        $('#depositCreateSubmitBtn').prop('disabled', false);

                        return;
                    } else {
                        document.getElementById('depositCreateForm').submit();
                    }
                },
