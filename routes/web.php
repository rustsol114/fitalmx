...
Route::any('/webhooks/stp/crypto-account-status', 'WebhookController@handleStpStatus');
Route::any('/webhooks/stp/state-change', 'WebhookController@handleNotification');
Route::any('/webhooks/stp/new-payment', 'WebhookController@handlePayment');
...
Route::group(['middleware' => ['no_auth:users', 'locale']], function () {
    ...
    Route::get('redirect-back', function () {
        return redirect(env('REDIRECT_TO_LANDING_URL','https://fitalmx.corp.merehead.xyz/'));
...
Route::group(['middleware' => ['guest:users', 'locale', 'twoFa', 'check-user-inactive'], 'namespace' => 'Users'], function () {
    ...
    Route::group(['middleware' => ['permission:manage_deposit', 'check-user-suspended']], function () {
        ...
        Route::post('deposit/get-user-info-from-stp', 'DepositController@getStpUserInfo')->name('user.deposit.info-from-bank');

        Route::get('deposit/stp-form', 'DepositController@stpForm')->name('user.deposit.stp-form');
        Route::post('deposit/stp-register', 'DepositController@stpRegister')->name('user.deposit.stp_register');
