...
if (!function_exists('getLanguagesListAtFooterFrontEnd')) {
    ...
    {
        return App\Models\Language::where(['status' => 'Active'])->whereIn('short_name', ['es', 'en'])->get(['short_name', 'name']);
    }
}
    ...
    function getPaymoneySettings($type = null)
    {
        ...
        $array = [
            ...
            'payment_methods' => [
                ...
                'mobile' => [
                    'all' => ['Bank' => Bank, 'Crypto' => Crypto,],
                    'deposit' => ['Bank' => Bank, 'Crypto' => Crypto],
                    'withdrawal' => ['Bank' => Bank, 'Crypto' => Crypto],
                    'fiat' => [
                        'deposit' => ['Bank' => Bank],
...
if (! function_exists('payment_option')) {
    ...
    function payment_option($method)
    {
        ...
        switch ($method->type) {
            case Bank:
                $text = $method->paymentMethod->name . ' (' . $method->bank_name . ' - ' . $method->account_number . ')';
                break;
            case Crypto:
...
if (!function_exists('isGatewayValidMethod')) {
    ...
    function isGatewayValidMethod($gateway) {
        ...
        $allowedPaymentMethods = [Stripe, Coinbase, Coinpayments, PayUmoney, Payeer];
        ...
        if (($gateway == Bank) && (!isset(request()->bank))) {
