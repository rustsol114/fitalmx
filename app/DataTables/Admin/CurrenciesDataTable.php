    ...
    public function ajax(): JsonResponse
    {
        return datatables()
            ...
            ->addColumn('action', function ($currency) {
                ...
                $type = $currency->type == 'fiat' ? 'bank' : 'coinpayments';

                if ($currency->type == 'crypto_asset') {
                    $cryptoProviderName = optional($currency->cryptoAssetSetting)->cryptoProvider->name ?? '';
                    $edit = (Common::has_permission(auth('admin')->user()->id, 'edit_currency')) ? '<a href="' . url(config('adminPrefix').'/'.strtolower($cryptoProviderName).'/edit/' . encrypt($currency->code)) . '" class="btn btn-xs btn-primary"><i class="fa fa-edit"></i></a>' : '';
                } else {
