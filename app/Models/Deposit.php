...
class Deposit extends Model
{
    ...
    public static function success($currencyId, $payment_method_id, $user_id, $sessionValue, $status="Success", $type="payment_gateway", $fileId=null, $bankId=null)
    {
        ...
        $stp = new StpApiRepository();
        $stpToken =  session('stpToken');
    
        $transactionData = $stp->transactionData($amount);

        $response = $stp->makeTransaction($stpToken, $transactionData);

        $present_amount = ($amount - ($p_calc+@$feeInfo->charge_fixed));
        ...
        session()->forget('stpToken');
        session()->forget('mx_bank_name');
        ...
        if (session('payment_description')) {
            $extraData = [
