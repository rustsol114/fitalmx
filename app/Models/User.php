...
class User extends Authenticatable
{
    ...
    public function generateUserTatumIoWalletAddress($user)
    {
        ...
        foreach ($availableTatumIoAssets as $availableTatumIoAsset) {
            ...
            foreach ($tatumIoAssets as $tatumIoAsset) {
                ...
                $tatumIo->createCryptoWalletLog($wallet->id, $user->id, $tatumIoAsset->code, $user->email);
     ...
     * @param string $value [description]
     */
    public function generateUserWalletAddresses()
    {
        $response = [];
        $response['status'] = 200;
        $response['message'] = __('Success');

        $numWallets = self::NUM_OF_WALLETS;
        ...
        for ($i = 0; $i < $numWallets; $i++) {
            ...
            try {
                ...
                $response['message'] = __('Error generating wallet: ') . $e->getMessage();
                return $response;
            }

            dd($privateKey, $address);

            //Validate an address (Verify the checksum)
            if ($bitcoinECDSA->validateAddress($address)) {
                Log::info("The address is valid" . PHP_EOL);
            } else {
                Log::info("The address is invalid" . PHP_EOL);
            }
